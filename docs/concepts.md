...
Gamagama is built around a few core concepts that differentiate it from standard CLI tools.

## The Tree

Everything in Gamagama is a **Node** in a **Tree**.
...
If you try to run a command that doesn't exist on your Current Node, Gamagama looks up the tree (to the parent, then the grandparent, etc.) until it finds a handler.
